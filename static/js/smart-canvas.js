...
async function runGeneration(){
    ...
    try {
        ...
        if(isApiLikeEngine(settings.engine) && settings.apiKind === 'video'){
            ...
            if(outVideos && outVideos.pendingVideoTaskId){
                setNodeVideoTask(pendingNode, outVideos.pendingVideoTaskId);
                if(sourceVisualState) restoreSourceVisualState(node, sourceVisualState);
...
async function runApiVideoGeneration(prompt, refs, runSettings=settings){
    ...
    try {
        ...
        // 通用视频异步任务：后端已提交到后台轮询，返回 task_id 供前端轮询恢复（刷新/切画布不丢）
        if(result && result.status === 'pending' && result.task_id){
            return {pendingVideoTaskId:result.task_id, message:result.message || ''};
        }
...
// --- 通用视频异步任务：进度持久化 + 刷新/切画布自动恢复（带超时护栏）---
const VIDEO_TASK_POLL_INTERVAL = 8000;   // 轮询间隔（毫秒）
const VIDEO_TASK_POLL_MAX = 450;         // 最大轮询次数（450 * 8s ≈ 60 分钟，超过即放弃恢复，避免刷新增噪）
const VIDEO_TASK_RESUME_TTL = 6 * 3600 * 1000; // 重新打开页面时只恢复 6 小时内的任务
const activeVideoTaskPolls = new Set();
function setNodeVideoTask(node, taskId, message=''){
    ...
    node.pending = 0;
    delete node.pendingTasks;
    if(!node.runStartedAt) node.runStartedAt = node.videoTask.startedAt;
    delete node.runFinishedAt;
    node.runTimerHidden = false;
    render();
    scheduleSave();
    startVideoTaskPoll(node);
}
...
function applyVideoTaskStatus(node, data){
    if(!node || !data || !node.videoTask) return false;
    if(data.status === 'succeeded'){
        const urls = (data.videos || []).map(url => ({url, kind:'video', generatedResult:true, name:'output.mp4'}));
        if(!urls.length) return false;
        delete node.videoTask;
        replaceOutputsToNodeWithHistory(node, urls, 'video', null, {skipShift:true});
        node.running = false;
        node.pending = 0;
        node.runFinishedAt = nowMs();
    ...
    if(data.status === 'failed'){
        ...
        node.running = false;
        node.pending = 0;
        toast((data.detail || data.error || '视频生成失败').slice(0, 160));
        render();
        scheduleSave();
        return true;
    }
...
function startVideoTaskPoll(node){
    if(!node || !node.videoTask || !node.videoTask.taskId) return;
    const taskId = node.videoTask.taskId;
    if(activeVideoTaskPolls.has(taskId)) return;
    activeVideoTaskPolls.add(taskId);
    const nodeId = node.id;
    (async () => {
        try {
            for(let i = 0; i < VIDEO_TASK_POLL_MAX; i++){
                await new Promise(resolve => setTimeout(resolve, VIDEO_TASK_POLL_INTERVAL));
                const cur = nodes.find(n => n.id === nodeId);
                if(!cur || !cur.videoTask || cur.videoTask.taskId !== taskId) return; // 已被别处处理/清空
                if(cur.videoTask.querying) continue;
                let data;
                try {
                    data = await fetchVideoTaskStatus(taskId);
                } catch(err){ continue; }
                if(data.status === 'gone'){
                    // 服务端缓存已清空（重启/过期），放弃恢复以免误导为永久失败
                ...
                const done = applyVideoTaskStatus(cur, data);
                if(done) return;
                const after = nodes.find(n => n.id === nodeId);
                if(!after || !after.videoTask || after.videoTask.taskId !== taskId) return;
            }
        } finally {
            activeVideoTaskPolls.delete(taskId);
        }
    })();
}
function resumeVideoTaskNodes(){
    nodes.filter(n => n && n.videoTask && n.videoTask.taskId).forEach(n => {
        // 超时护栏：超过 6 小时的服务端任务大概率为过期清除，放弃恢复
        if(nowMs() - Number(n.videoTask.startedAt || 0) > VIDEO_TASK_RESUME_TTL){
            delete n.videoTask;
            n.running = false;
            n.pending = 0;
            render();
