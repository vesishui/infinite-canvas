...
function renderNode(node){
    ...
    const statusHtml = showStatus ? (() => {
        ...
        const errText = node.runStatus === 'failed' && node.runError ? `<span class="node-run-error-text">${escapeHtml(node.runError.slice(0, 120))}</span>` : '';
        return `<span class="node-run-status ${node.runStatus}"><span class="dot"></span>${escapeHtml(label)}${node._cascadeIdx?' '+node._cascadeIdx:''}</span>${errText}`;
    })() : '';
...
async function runVideoNode(nodeId, opts={}){
    ...
    try {
        ...
        // 异步提交（task_id）：写入节点并启动恢复轮询，刷新/切画布不丢进度
        console.log('[VIDEO-FRONTEND] canvas-video response:', JSON.stringify(result).slice(0, 300));
        if(result && result.status === 'pending' && result.task_id){
            console.log('[VIDEO-FRONTEND] → async path taken, task_id=', result.task_id);
            // 不要立即删 pending 占位，让它在视频最终成功/失败时再处理（OUTPUT 节点保留"生成中"占位卡片）
            if(out && out._pending){
                out._pending.forEach(p => { if(p.id === pendingId) p.recoverTaskId = result.task_id; });
            }
            setVideoNodeTask(node, result.task_id, result.message || '');
            if(opts.cascade) throw new CascadeAbortError('video pending');
            return;
        }
        console.log('[VIDEO-FRONTEND] → sync path, result keys=', Object.keys(result || {}));
        const meta = collectRunMeta(out, pendingId);
        if(out) out._pending = (out._pending || []).filter(p => p.id !== pendingId);
        ...
        console.log('[VIDEO-FRONTEND] runVideoNode error:', err.message || String(err), 'hasVideoTask=', !!node.videoTask);
        const meta = collectRunMeta(out, pendingId);
        addGenerationLog({run, outputs:[], runMs:meta.runMs || 0, error:err.message || String(err)});
        if(out) out._pending = (out._pending || []).filter(p => p.id !== pendingId);
        if(isCascadeAbortError(err)){
        ...
        // 异步路径（videoTask 已由 setVideoNodeTask 设置并启动轮询）不要覆盖 running 状态
        if(!node.videoTask){
            node.running = false;
        }
        refreshRunNodes(node, out);
...
// --- 经典画布：视频异步任务进度持久化 + 刷新/切画布自动恢复（带超时护栏）---
const CANVAS_VIDEO_POLL_INTERVAL = 8000;
const CANVAS_VIDEO_POLL_MAX = 450;
const CANVAS_VIDEO_RESUME_TTL = 6 * 3600 * 1000;
const activeCanvasVideoPolls = new Set();
function fetchCanvasVideoTaskStatus(taskId){
    return fetch(`/api/canvas-video/${encodeURIComponent(taskId)}`).then(async r => {
        if(r.status === 404) return {status:'gone'};
        if(!r.ok){
            // 后端在任务失败时返回 502 + detail，把 detail 透出给 UI，避免前端被吞掉继续轮询
            let detail = '';
            try { const j = await r.json(); detail = j?.detail || j?.error || ''; } catch(_){ try { detail = await r.text(); } catch(_){} }
            return {status:'failed', detail: detail || '视频生成失败'};
        }
        return r.json();
    });
}
function setVideoNodeTask(node, taskId, message=''){
    ...
    scheduleSave();
    startCanvasVideoPoll(node);
}
function markOutputPendingFailedForTask(node, taskId, errorMsg){
    if(!node || !taskId) return;
    const out = outputForNode(node, 460);
    if(!out || !out._pending) return;
    const target = out._pending.find(p => p.recoverTaskId === taskId);
    if(target){
        target.failed = true;
        target.error = errorMsg || '视频生成失败';
        target.querying = false;
        refreshRunNodes(out);
    }
}
function finalizeCanvasVideoTask(node, videos){
    const out = outputForNode(node, 460);
    const items = (videos || []).map(url => ({url, kind:'video', generatedResult:true, name:'output.mp4'}));
    if(!items.length) return false;
    const finishedTaskId = node.videoTask?.taskId || '';
    delete node.videoTask;
    mergeGeneratedOutputs(node, items, false);
    if(out) appendOutputImages(out, items, null, [{kind:'video'}]);
    // 清理 OUTPUT 上对应的 pending 占位卡片（异步任务期间保留，成功时移除）
    if(out && finishedTaskId && Array.isArray(out._pending)){
        const before = out._pending.length;
        out._pending = out._pending.filter(p => p.recoverTaskId !== finishedTaskId);
        if(out._pending.length !== before) refreshRunNodes(out);
    }
    node.runStatus = 'done'; node.runError = '';
    node.running = false;
    refreshRunNodes(node, out);
...
function applyCanvasVideoTaskStatus(node, data){
    if(!node || !data || !node.videoTask) return false;
    if(data.status === 'succeeded'){
        return finalizeCanvasVideoTask(node, data.videos || []);
    }
    if(data.status === 'failed'){
        const failedTaskId = node.videoTask?.taskId || '';
        const failedMsg = data.detail || data.error || '视频生成失败';
        delete node.videoTask;
        node.running = false;
        node.runStatus = 'failed';
        node.runError = failedMsg;
        // 同步把 OUTPUT 节点上的 pending 占位标记为 failed，避免一直显示"生成中"
        markOutputPendingFailedForTask(node, failedTaskId, failedMsg);
        refreshRunNodes(node);
        scheduleSave();
        return true;
    }
    if(node.videoTask){
        node.videoTask.updatedAt = nowMs();
        if(data.message) node.videoTask.message = data.message;
    }
    refreshRunNodes(node);
    scheduleSave();
    return false;
}
function startCanvasVideoPoll(node){
    if(!node || !node.videoTask || !node.videoTask.taskId) return;
    const taskId = node.videoTask.taskId;
    if(activeCanvasVideoPolls.has(taskId)) return;
    activeCanvasVideoPolls.add(taskId);
    const nodeId = node.id;
    (async () => {
        try {
            for(let i = 0; i < CANVAS_VIDEO_POLL_MAX; i++){
                await new Promise(resolve => setTimeout(resolve, CANVAS_VIDEO_POLL_INTERVAL));
                const cur = nodes.find(n => n.id === nodeId);
                if(!cur || !cur.videoTask || cur.videoTask.taskId !== taskId) return;
                if(cur.videoTask.querying) continue;
                let data;
                try { data = await fetchCanvasVideoTaskStatus(taskId); }
                catch(err){ continue; }
                if(data.status === 'gone'){
                    delete cur.videoTask;
                    cur.running = false;
                    cur.runStatus = 'failed';
                    cur.runError = '视频任务已过期（服务端重启/缓存清空），请重新生成';
                    refreshRunNodes(cur);
                    scheduleSave();
                    return;
                }
                const done = applyCanvasVideoTaskStatus(cur, data);
                if(done) return;
                const after = nodes.find(n => n.id === nodeId);
                if(!after || !after.videoTask || after.videoTask.taskId !== taskId) return;
            }
        } finally {
            activeCanvasVideoPolls.delete(taskId);
        }
    })();
}
function resumeCanvasVideoTasks(){
    nodes.filter(n => n && n.videoTask && n.videoTask.taskId).forEach(n => {
        if(nowMs() - Number(n.videoTask.startedAt || 0) > CANVAS_VIDEO_RESUME_TTL){
            delete n.videoTask;
            n.running = false;
            n.runStatus = 'failed';
            n.runError = '视频任务已超时，请重新生成';
            refreshRunNodes(n);
            scheduleSave();
            return;
        }
        n.videoTask.querying = false;
        n.running = true;
        n.runStatus = 'running';
        refreshRunNodes(n);
        startCanvasVideoPoll(n);
    });
}
...
function completeRecoverPendingOutput(out, pending, result){
    ...
    // 视频任务返回 videos 数组，图片任务返回 images 数组
    const images = result.images || [];
    const videos = result.videos || [];
    const items = videos.length ? videos.map(v => ({url:v.url||v, kind:'video', generatedResult:true, name:'output.mp4'})) : images;
    if(!items.length) return;
    const meta = {
    ...
    if(videos.length){
        appendOutputImages(out, items, null, [meta]);
    } else {
        appendOutputImages(out, images, meta.run?.refs?.[0], [meta]);
    }
    const gen = nodes.find(n => n.id === meta.run?.node?.id);
    if(gen){
        mergeGeneratedOutputs(gen, items, Boolean(pending.appendGenerated));
        gen.runStatus = 'done';
        gen.runError = '';
        gen.running = false;
    }
    addGenerationLog({run:meta.run, outputs:items, runMs:meta.runMs || 0});
    refreshRunNodes(gen, out);
    scheduleSave();
}
async function queryRecoverPendingOutput(pendingId){
    ...
    refreshNodes([out.id]);
    try {
        // 视频任务走 canvas-video 查询端点，图片任务走 image-task-query
        const isVideo = pending.kind === 'video' || taskId.startsWith('task_');
        let data;
        if(isVideo){
            const res = await fetch(`/api/canvas-video/${encodeURIComponent(taskId)}`);
            if(res.status === 404) throw new Error('视频任务不存在或已过期');
            if(!res.ok && res.status !== 502){
                throw new Error(await responseErrorMessage(res, '查询失败'));
            }
            data = await res.json();
            // 502 时 data = {detail: "错误信息"}，当作 failed 处理
            if(res.status === 502){
                data = {status:'failed', error: data.detail || '视频生成失败'};
            }
        } else {
        ...
        if(data.status === 'failed'){
            pending.error = data.detail || data.error || tr('canvas.generationFailed');
