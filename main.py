...
# 视频上游轮询间隔：部分上游（如灵创）要求「每 10 秒查询一次，
# 任务仍 queued/submitting/processing 时不要重复提交」，过频会触发限流/任务失败。
# 故视频轮询起步即 10s，且最多不超过 15s。
VIDEO_POLL_INTERVAL = float(os.getenv("VIDEO_POLL_INTERVAL", "10"))
VIDEO_POLL_INTERVAL_MAX = float(os.getenv("VIDEO_POLL_INTERVAL_MAX", "15"))
...
def is_seed4k_provider(provider):
    """seed-4k：标准 OpenAI 图片协议（/images/generations + /images/edits），
    图生图用顶层 JSON image 字段（base64 data URL），响应固定 b64_json。
    必须单独处理，因为 openai-json 分支用 extra_body.image 包裹参考图，
    seed-4k 不认 extra_body，会当文生图处理或直接报错。"""
    if not isinstance(provider, dict):
        return False
    try:
        host = (urllib.parse.urlsplit(str(provider.get("base_url") or "").strip()).hostname or "").lower()
    except Exception:
        host = ""
    pid = str(provider.get("id") or "").strip().lower()
    return "seed-4k.com" in host or pid == "custom-api-5"
...
async def generate_seed4k_image(prompt, size, quality, model, reference_images, provider, aspect_ratio="", resolution=""):
    """seed-4k 专用图片生成（按官方文档）：
    - 无参考图 → POST /images/generations（文生图）
    - 有参考图 → POST /images/edits + 顶层 JSON image 字段（base64 data URL）
    - 响应固定 b64_json，用现有 extract_image 解析。
    不能用 openai-json 的 extra_body.image 包裹（seed-4k 不认，会当文生图处理）。
    """
    base_url = str((provider or {}).get("base_url") or "").strip().rstrip("/")
    if not base_url:
        raise HTTPException(status_code=400, detail=f"{provider.get('name') or provider.get('id') or 'seed-4k'} 未配置 Base URL")
    if aspect_ratio:
        try:
            size = seed4k_size_from_aspect(size, aspect_ratio)
        except Exception:
            pass
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(connect=20.0, read=1800.0, write=120.0, pool=20.0)) as client:
        if image_b64:
            # 图生图：/images/edits + 顶层 JSON image 字段（seed-4k 官方要求 base64 用 JSON 的 image 字段）
            edits_url = provider_endpoint_url(provider, "image_edit_endpoint", "/v1/images/edits")
            body["image"] = image_b64
            response = await client.post(edits_url, headers=headers, json=body)
        else:
...
def seed4k_size_from_aspect(size, aspect_ratio):
    """把画布宽高比换算成 seed-4k 认可的尺寸（长边 1024 / 2K 由 resolution 决定）。"""
    s = str(size or "").strip().lower()
    if s and s not in {"auto", "1:1"}:
        return s
    ...
    return maps.get(ratio, "1024x1024")
...
def is_xinfeng_provider(provider):
    # 新峰(xinfeng.best)：视频走 OpenAI 视频协议 POST /v1/videos（seconds/size/watermark），
    # 参考图走 multipart input_reference；区别于默认兜底的 /v1/videos/generations。
    base_url = str((provider or {}).get("base_url") or "").lower()
    provider_id = str((provider or {}).get("id") or "").strip().lower()
...
def is_lingchuang_provider(provider):
    ...
    # 写错路径会被服务端以 404 异常断连，表现为 BrokenPipeError。需单独识别。
    base_url = str((provider or {}).get("base_url") or "").lower()
    provider_id = str((provider or {}).get("id") or "").strip().lower()
    video_models = (provider or {}).get("video_models") or []
    model_blob = " ".join(str(m) for m in video_models).lower()
    return (
        "lingchuangai.top" in base_url
        or provider_id in ("custom-api-3", "lingchuang")
        or "lc-" in model_blob
    )
...
async def upload_video_to_kingimage(path: str, source_url: str) -> Dict[str, str]:
    """免费图床 kingimage (file.kxlove.top)，北京网络下直链可访问，无需 token。

    接口：POST https://file.kxlove.top/api.php
    表单字段：file(必填) / show(0不显示) / ispwd(0无密码)
    成功响应：{"code":0,"downurl":"直链","viewurl":"预览页"}
    code 非 0 表示失败；downurl 才是真实图片直链（viewurl 是网页预览，不能给视频API）。
    """
    upload_url = os.getenv("KINGIMAGE_UPLOAD_URL", "https://file.kxlove.top/api.php").strip() or "https://file.kxlove.top/api.php"
    ct = content_type_for_path(path)
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(connect=20.0, read=600.0, write=600.0, pool=20.0), follow_redirects=True) as client:
            with open(path, "rb") as fh:
                ...
                response = await client.post(upload_url, data=data, files=files)
        if not response.is_success:
            raise HTTPException(status_code=response.status_code, detail=f"KingImage 上传失败：{response.text[:300]}")
        try:
            payload = response.json()
        except Exception:
            raise HTTPException(status_code=502, detail=f"KingImage 返回了非 JSON 内容：{response.text[:300]}")
        code = payload.get("code")
        if code != 0 and str(code) != "0":
            raise HTTPException(status_code=502, detail=f"KingImage 上传被拒绝(code={code})：{payload.get('msg', response.text[:200])}")
        direct_url = (payload.get("downurl") or "").strip()
        if not re.match(r"^https?://", direct_url, re.I):
            raise HTTPException(status_code=502, detail=f"KingImage 未返回直链(downurl)：{response.text[:300]}")
        return {"url": direct_url, "source": source_url, "name": os.path.basename(path), "expires": "unknown", "service": "kingimage"}
    except HTTPException:
        raise
    except Exception as exc:
        raise HTTPException(status_code=502, detail=f"KingImage 上传异常：{exc}") from exc
...
async def upload_local_video_to_cloud(ref_url: str, service: str = "auto") -> Dict[str, str]:
    ...
    if service in {"kingimage", "king", "aa1", "aa1cn"}:
        return await upload_video_to_kingimage(path, ref_url)
    errors = []
    for name, func in (("kingimage", upload_video_to_kingimage), ("litterbox", upload_video_to_litterbox), ("temp.sh", upload_video_to_temp_sh)):
        try:
...
async def generate_ai_image(prompt, size, quality, model, reference_images=None, provider_id="comfly", aspect_ratio="", resolution=""):
    ...
    if is_seed4k_provider(provider):
        return await generate_seed4k_image(prompt, size, quality, model, reference_images, provider, aspect_ratio, resolution)
...
VIDEO_URL_KEYS = (
    ...
    "result_url", "resultUrl", "cover_url", "coverUrl",  # 灵创AI：完成字段为 result_url / cover_url
)
...
def video_submit_url_candidates(provider, base_url):
    if is_lingchuang_provider(provider):
        # 灵创AI 视频端点为单数 /v1/video/generations（OpenAI 兼容但非复数写法）
...
def video_task_url_candidates(provider, base_url, task_id, submit_url=""):
    if is_lingchuang_provider(provider):
        return [f"{base_url}/v1/video/generations/{task_id}"]
    if is_tudou_provider(provider):
...
async def wait_for_video_task(client, provider, task_id, submit_url=""):
    ...
    while time.monotonic() < deadline:
        ...
        if status in VIDEO_TASK_FAILURE_STATUSES:
            ...
            print(f"[VIDEO-POLL] 上游返回失败 task_id={task_id} upstream_status={status} raw={str(raw)[:500]}", flush=True)
            raise HTTPException(status_code=502, detail=humanize_video_task_failure(reason))
        delay = min(delay + 1.0, VIDEO_POLL_INTERVAL_MAX)
...
async def generate_xinfeng_video(client, payload, provider, base_url, requested_model):
    """新峰(xinfeng.best) OpenAI 视频协议：POST /v1/videos，seconds/size/watermark，
    参考图走 multipart input_reference（本地图直接读字节上传，无需图床公网化）。"""
    submit_url = f"{base_url}/v1/videos"
    data = {
    ...
    try:
        ...
        resp_text = (response.text or "")[:500]
        raise HTTPException(status_code=502, detail=f"新峰视频接口返回非 JSON 响应（状态 {response.status_code}）：{resp_text}") from exc
    task_id = str(raw.get("id") or extract_task_id(raw) or raw.get("task_id") or "").strip()
    result = raw
    if task_id and not video_output_urls(raw):
        result = await wait_for_video_task(client, provider, task_id, submit_url)
    urls = video_output_urls(result)
    if not urls:
        raise HTTPException(status_code=502, detail=f"新峰视频生成成功但没有返回视频：{result}")
    local_urls = [await save_remote_video_to_output(url) for url in urls]
    return {"videos": local_urls, "task_id": task_id, "raw": result}
...
# 视频异步任务进度缓存：task_id -> {status, videos, error, provider_id, started_at}
# 仅保存在内存中，服务重启即清空（前端恢复时有超时护栏，不会无限重试）。
CANVAS_VIDEO_TASKS = {}

async def _background_video_poll(provider, task_id, submit_url):
    """提交后由 asyncio 后台任务独立轮询视频生成并落盘，断连也不丢。"""
    entry = CANVAS_VIDEO_TASKS.setdefault(task_id, {"status": "pending", "provider_id": provider.get("id")})
    try:
        async with httpx.AsyncClient(timeout=VIDEO_POLL_TIMEOUT) as client:
            result = await wait_for_video_task(client, provider, task_id, submit_url)
        urls = video_output_urls(result)
        if not urls:
            entry.update({"status": "failed", "error": f"视频生成成功但没有返回视频：{result}"})
            return
        # 完成时落盘：远程 URL -> 本地 /api/storage-files/generated/... 永久保住
        local_urls = [await save_remote_video_to_output(url) for url in urls]
        entry.update({"status": "succeeded", "videos": local_urls, "raw": result})
        print(f"[VIDEO-TASK] 视频生成完成 task_id={task_id} 本地路径={local_urls}", flush=True)
    except Exception as exc:
        detail = getattr(exc, "detail", None) or str(exc)
        entry.update({"status": "failed", "error": str(detail)})
        print(f"[VIDEO-TASK] 视频生成失败 task_id={task_id} error={detail}", flush=True)
...
async def canvas_video(payload: CanvasVideoRequest):
    ...
    if is_xinfeng:
        try:
            async with httpx.AsyncClient(timeout=VIDEO_POLL_TIMEOUT) as xinfeng_client:
                return await generate_xinfeng_video(xinfeng_client, payload, provider, base_url, requested_model)
        except httpx.HTTPStatusError as exc:
            text = exc.response.text
            raise HTTPException(status_code=exc.response.status_code, detail=f"新峰视频接口错误：{text}") from exc
        except httpx.HTTPError as exc:
            log_net_error(f"视频(新峰) 网络/TLS错误 model={requested_model}", exc)
            raise HTTPException(status_code=502, detail=f"请求新峰视频接口失败：{exc}") from exc
        ...
        async with httpx.AsyncClient(timeout=VIDEO_POLL_TIMEOUT) as client:
            ...
            if is_apimart:
                ...
                if is_volcengine and not volc_is_proxy:
                    ...
                    image_payload = []
                    for idx, ref in enumerate(payload.images[:4]):
                        if not ref.url:
                            continue
                        if is_lingchuang_provider(provider):
                            # 灵创只认公网 http(s) URL：本地图走图床公网化（kingimage/穿透），失败回退 base64
                            try:
                                pub_url = await openai_video_proxy_public_reference_url(ref.dict())
                                print(f'[VIDEO-UPLOAD] 图{idx+1} 公网化成功: {ref.url[:60]} → {pub_url[:120]}')
                                image_payload.append(pub_url)
                            except HTTPException as exc:
                                print(f'[VIDEO-UPLOAD] 图{idx+1} 公网化失败({exc.detail[:100]}), 回退base64: {ref.url[:60]}')
                                image_payload.append(reference_to_data_url(ref.dict(), max_size=1536))
                            except Exception as exc2:
                                print(f'[VIDEO-UPLOAD] 图{idx+1} 公网化异常({exc2}), 回退base64: {ref.url[:60]}')
                                image_payload.append(reference_to_data_url(ref.dict(), max_size=1536))
                        else:
                            image_payload.append(reference_to_data_url(ref.dict(), max_size=1536))
                    # 打印最终发给灵创的参考图列表，方便排查 generation_failed
                    if is_lingchuang_provider(provider) and image_payload:
                        print(f'[VIDEO-UPLOAD] reference_image_urls ({len(image_payload)}张):')
                        for u_idx, u in enumerate(image_payload):
                            preview = u[:150] if len(u) > 150 else u
                            is_https = preview.startswith('https://')
                            is_data = preview.startswith('data:')
                            is_local = not is_https and not data and not preview.startswith('http')
                            tag = '✅HTTPS' if is_https else ('⚠️BASE64' if is_data else ('❌OTHER' if is_local else '❓'))
                            print(f'  [{u_idx+1}] {tag} {preview}')
                    body = {
                        "prompt": payload.prompt,
                    ...
                    if payload.videos:
                        if is_lingchuang_provider(provider):
                            body["reference_video_urls"] = [v for v in payload.videos if v]
                        else:
            ...
            print(f"[VIDEO-DEBUG] provider={provider.get('id')} raw={str(raw)[:600]} task_id={task_id} has_videos={bool(video_output_urls(raw))}", flush=True)
            result = raw
            if task_id and not video_output_urls(raw):
                # 非阻塞：若上游是异步任务（只返回 task_id，无视频 URL），
                # 不再在当前 HTTP 连接内同步轮询（刷新/切画布会断连丢失），
                # 改为启动后台任务独立轮询+落盘，前端用 task_id 轮询恢复。
                if not is_yuli_provider(provider):
                    asyncio.create_task(_background_video_poll(provider, task_id, submit_url))
                    CANVAS_VIDEO_TASKS.setdefault(task_id, {
                        "status": "pending",
                        "provider_id": provider.get("id"),
                        "started_at": time.time(),
                    })
                    print(f"[VIDEO-TASK] 已提交异步视频任务 task_id={task_id} provider={provider.get('id')} model={payload.model}", flush=True)
                    return {
                        "task_id": task_id,
                        "status": "pending",
                        "message": "视频生成任务已提交，正在后台生成（可刷新或切换画布，进度会自动恢复）",
                    }
                # 玉玉 API 仍保持原同步轮询行为
                result = await wait_for_video_task(client, provider, task_id, submit_url)
            urls = video_output_urls(result)
...
@app.get("/api/canvas-video/{task_id}")
async def canvas_video_status(task_id: str):
    """查询后台视频生成任务状态。前端用此端点轮询恢复（刷新/切画布不丢进度）。"""
    entry = CANVAS_VIDEO_TASKS.get(task_id)
    if not entry:
        # 服务重启/缓存已清空：让前端用"超时护栏"自然放弃，避免误导为永久失败
        raise HTTPException(status_code=404, detail="视频任务不存在或已超出缓存（服务端重启/过期）。请重新提交生成。")
    ...
    if status == "failed":
        raise HTTPException(status_code=502, detail=entry.get("error", "视频生成失败"))
